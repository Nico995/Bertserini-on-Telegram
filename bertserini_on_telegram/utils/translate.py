...
class AutoTranslator:
    ...
    def translate(self, text: str, src_lang: str = None, trg_lang: str = None) -> Tuple[str, Tuple[str, str]]:
        ...
        lang_iso369 = self.fasttext.predict(text)[0][0][-2:]

        if not src_lang:
            src_lang = self.lang_map.get(lang_iso369, None)
        ...
        if not src_lang:
            NotImplementedError(f'The language with iso369 code of {lang_iso369} is not suported from Mbart50-m2m')
            return None, (src_lang, trg_lang)
